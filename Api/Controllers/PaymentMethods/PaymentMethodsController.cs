...
// Controlador encargado de exponer por HTTP las operaciones relacionadas con PaymentMethods.
public sealed class PaymentMethodsController : BaseApiController
{
    ...
    {
        var result = await Sender.Send(new GetPaymentMethodsPaged(pageNumber, pageSize, search), ct);
        Response.Headers["X-Total-Count"] = result.TotalCount.ToString();
        return Ok(result);
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct) => Ok(await Sender.Send(new GetPaymentMethodById(id), ct));
}
